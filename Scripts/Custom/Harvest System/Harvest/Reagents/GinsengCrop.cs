...
namespace Server.Items.Crops
{
    public class GinsengSeed : BaseCrop
    {
        ...
        {
            from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0x3B2, 3, "", String.Format("{0} small dull brown seed{1}", Amount > 1 ? Amount.ToString() : "a", Amount > 1 ? "s" : "")));
        }
    ...
    public class GinsengCrop : BaseCrop
    {
        ...
        {
            from.SendMessage("The ginseng plant withers away.");
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 1))
                {
                    ...
                    else //check skill and sower
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You manage to gather {0} ginseng!", pick);

                        double chance = Utility.RandomDouble(); // chance to get a seed is 20%
                        if (0.20 > chance)
                        {
                            from.AddToBackpack(new GinsengSeed(1));
                        ...
                        { 
                            from.SendMessage("You accidently uproot all of the ginseng plant.");
                            regrowTimer.Stop();
                            this.Delete();
                            return;
                        }
