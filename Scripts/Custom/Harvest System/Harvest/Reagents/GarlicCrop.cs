...
{ 
	...
	{ 
		...
		[Constructable]
		public GarlicSeed( int amount ) : base( 0xDCF )
		{
            this.Name = "galic seed";
			Stackable = true; 
			Weight = .5; 
			Hue = 1151; 
			Movable = true; 			
        ...
        {
            from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0x3B2, 3, "", String.Format("{0} small white seed{1}", Amount > 1 ? Amount.ToString() : "a", Amount > 1 ? "s" : "")));
        }
	...
	{ 
		...
		{
			from.SendMessage( "The garlic plant withers away." ); 
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( GarlicCrop plant ) : base(TimeSpan.FromMinutes(45), TimeSpan.FromMinutes(30))
			{
