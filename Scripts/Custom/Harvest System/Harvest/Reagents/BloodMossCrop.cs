...
{ 
	...
	{ 
		...
		[Constructable]
		public BloodMossSeed( int amount ) : base( 0xDCF )
		{
            this.Name = "blood mose seed";
			Stackable = true; 
			Weight = .5; 
			Hue = 39; 
			Movable = true; 			
        ...
        {
            from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0x3B2, 3, "", String.Format("{0} dull red seed{1}", Amount > 1 ? Amount.ToString() : "a", Amount > 1 ? "s" : "")));
        }
	...
	{ 
		...
		[Constructable] 
		public BloodMossSeedling( Mobile sower ) : base( 0xD11 )
		{ 
			Movable = false; 
			Name = "a bloodmoss seedling"; 
			m_sower = sower;
            Hue = 39;
		...
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BloodMossCrop), plant.Sower ); 
	...
	public class BloodMossCrop : BaseCrop 
	{ 
		...
		[Constructable] 
		public BloodMossCrop( Mobile sower ) : base( 0xC69 ) 
		{ 
			Movable = false; 
			Name = "a bloodmoss plant";
            Hue = 39;
			m_sower = sower;
		...
		public static void init ( BloodMossCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xD12 );
		...
		{
			from.SendMessage( "The bloodmoss plant withers away." ); 
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill and sower
					{
						...
						m_yield -= pick;
						from.SendMessage( "You manage to gather {0} bloodmoss!", pick);

                        double chance = Utility.RandomDouble(); // chance to get a seed is 20%
                        if (0.20 > chance)
                        {
                            from.AddToBackpack(new BloodMossSeed(1));
                        ...
                        { 
                            from.SendMessage("You accidently uproot all of the bloodmoss plant.");
                            regrowTimer.Stop();
                            this.Delete();
                            return;
                        }
		...
		{
			private BloodMossCrop i_plant;

            public CropTimer(BloodMossCrop plant)
